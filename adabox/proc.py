"""
Local copy of the adaptive-boxes `proc` module so that OpenILT can import
...
def find_rectangle(find_rectangle_args: FindRectangleArgs):
    ...
    # Down
    final_x_min[0], final_x_max[0], final_y_down = get_final_xy_index_down(
        data_2d_arg, all_y_points, init_y_index, dist_l, dist_r, sep_value_arg
    )
    # Up
    final_x_min[1], final_x_max[1], final_y_up = get_final_xy_index_up(
        data_2d_arg, all_y_points, init_y_index, dist_l, dist_r, sep_value_arg
    )
...
def decompose(data_2d_global_arg, n_searches_per_step):
    ...
    while True:
        ...
        # Create args
        r_args = []
        for _ in range(n_searches_per_step):
